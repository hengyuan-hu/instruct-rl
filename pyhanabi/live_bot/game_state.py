...
import torch
import rela
import hanalearn as hle  # type: ignore
...
class HleGameState:
    def __init__(self, players, my_name, start_player, hide_action, verbose):
        ...
        self.beginning = True
        if self.verbose:
            print(f"Create state for {self.num_player} player game")
    ...
    def discard(self, seat, color, rank, order):
        card = Card(color, rank, order)
        if self.verbose:
            print('player %s discards [%s] ' % (seat, card))
        removed, card_idx = self.hands[seat].remove_from_hand(order)
        assert removed.order == order
    ...
    def _get_observation_and_legal_move(self):
        ...
        last_move = hle.get_last_non_deal_move(obs.last_moves())
        return obs_vec, legal_move_vec, last_move
    ...
    def observe(self):
        obs_vec, legal_move_vec, last_move = self._get_observation_and_legal_move()

        obs = torch.tensor(obs_vec, dtype=torch.float32)
        priv_s = obs.unsqueeze(0)
        publ_s = obs[125 * (self.num_player-1):].unsqueeze(0)
        legal_move = torch.tensor(legal_move_vec, dtype=torch.float32)

        return priv_s, publ_s, legal_move, last_move
